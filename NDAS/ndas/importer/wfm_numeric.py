...
class WFMNumericImporter(BaseImporter):
    """
    Importer for numeric waveform files
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs
        """
        super(WFMNumericImporter, self).__init__(*args, **kwargs)
    ...
    def get_dataframe(self, files):
        """
        Reads the contents of numeric waveform files into a dataframe
        ...
        for single_num_h in self.get_numeric_headers(files):
            self.log("Inspecting %s" % single_num_h)
            ...
            num_header = wfdb.rdrecord(single_num_h[:-4])

            self.custom_labels = ["NUM: " + str(n) + " (" + m + ")" for m, n in
                                  zip(num_header.units, num_header.sig_name)]
            self.custom_labels.insert(0, "sample (#)")

            df_num = pd.DataFrame.from_records(num_header.p_signal, columns=num_header.sig_name)
            df_num.insert(0, 'sample', range(num_header.sig_len))
    ...
    def get_labels(self, files):
        """
        Returns the labels for a loaded numeric waveform file
    ...
    def get_full_segment_path(self, segment_name, add_file_ending=False):
        """
        Returns the full path for numeric waveform files
    ...
    def get_numeric_headers(self, files):
        """
        Returns the header of numeric waveform files
        ...
        for file in files:
            ...
            if re.search("(\d\d\d\d-\d\d-\d\d-\d\d-\d\dn.hea)", file):
                num_header.append(file)
