...
class MainWindow(QMainWindow):
    ...
    def __init__(self, threadpool_obj, *args, **kwargs):
        ...
        self.stats_number_of_discovered_secondary_novelties_layout.addWidget(self.stats_number_of_discovered_secondary_novelties_label)
    ...
    def save_ndas_slot(self):
        ...
        file_name, _ = save_dialog.getSaveFileName(self, 'Choose save file location', '', 'NDAS Files (*.ndas)',
          options=options)
        if file_name:
            if file_name[-5:] != '.ndas':
    ...
    def load_ndas_slot(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        file_name, _ = QFileDialog.getOpenFileName(self, 'Choose NDAS file', '', 'NDAS Files (*.ndas)',
          options=options)
        if file_name:
    ...
    def _connect_signals(self):
        ...
        self.active_analysis_algorithm.currentIndexChanged.connect(lambda: self.change_algorithm_additional_parameters())
        self.toggle_hline_btn.clicked.connect(lambda: self.plot_layout_widget.set_h_line_visibility(self.toggle_hline_btn.isChecked()))
        self.toggle_vline_btn.clicked.connect(lambda: self.plot_layout_widget.set_v_line_visibility(self.toggle_vline_btn.isChecked()))
        ...
        self.annotation_add_label_btn.clicked.connect(lambda: self.plot_layout_widget.label_selection(self.annotation_active_label.currentText() + '|' + self.annotation_detailed_comment.text()))
        self.annotation_remove_label_btn.clicked.connect(lambda: self.plot_layout_widget.delabel_selection())
        ...
        self.plot_layout_widget.point_selection_changed_signal.connect(lambda num_selected: self.point_selection_changed_slot(num_selected))
        self.plot_layout_widget.point_labeling_changed_signal.connect(lambda num_labeled: self.point_labeling_changed_slot(num_labeled))
    ...
    def export_to_svg(self):
        ...
        file_name, _ = save_dialog.getSaveFileName(self, 'Choose file location', '', 'svg (*.svg)',
          options=options)
        if file_name:
            if file_name[-4:] != '.svg':
    ...
    def run_detection(self):
        ...
        algorithms.get_instance().signals.add_plot_signal.connect(lambda name, x_data, y_data, x_label, y_label: self.add_new_plot(name, x_data, y_data, x_label, y_label))
        algorithms.get_instance().signals.add_line_signal.connect(lambda plt_name, name, x_data, y_data: plots.add_line(plt_name, name, x_data, y_data))
        algorithms.get_instance().signals.add_infinite_line_signal.connect(lambda plt_name, name, y: plots.add_infinite_line(plt_name, name, y))
    ...
    def fm_open_csv_action(self):
        ...
        if file_name:
            ...
            data.get_instance().signals.result_signal.connect(lambda result_data, labels: self.data_import_result_slot(result_data, labels))
    ...
    def data_import_result_slot(self, df, labels):
        ...
        if data.get_dataframe_length() > data.truncate_size:
            self._no_confirm_info('Dataframe length exceeding size', 'Dataframe length is exceeding ' + str(data.truncate_size) + '. The dataframe set is truncated to ' + str(data.truncate_size) + ' data points and can be enlarged in the slicer settings.')
        self.data_selection_slider.setRangeLimit(data.data_slider_start, data.get_dataframe_length())
        self.data_selection_slider.valueChanged.connect(lambda start_value, end_value: self.update_data_selection_text(start_value, end_value))
        self.data_selection_start.textChanged.connect(lambda: self.update_data_selection_slider())
        self.data_selection_end.textChanged.connect(lambda: self.update_data_selection_slider())
    ...
    def fm_open_wfm_wave_action(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        file_names, _ = QFileDialog.getOpenFileNames(self, 'Choose Waveform Files', '', 'wfm files (*.hea)',
          options=options)
        if file_names:
    ...
    def _confirm_quit(self):
        ...
        """
        reply = QMessageBox.question(self, 'Message', 'Are you sure you want to exit the program?', QMessageBox.Yes, QMessageBox.No)
        if reply != QMessageBox.Yes:
            return False
        return True
